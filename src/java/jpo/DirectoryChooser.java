 ...
 */
 public class DirectoryChooser extends JPanel implements ActionListener{
	...
	/**
	 *   the drop down list of the previousely used directories
	 */
	...
	/**
	 *  the title for a JFileChooser window
	 */
	...
	public DirectoryChooser ( String chooserTitle, int validationType ) {
		...
		//for ( int i=0; i < Settings.maxCopyLocations; i++ ) {
	 	//	directoryJComboBox.addItem( makeObj( Settings.copyLocations[i] ) );
		//}

		for ( int i=0; i < Settings.copyLocations.length; i++ ) {
			//Tools.log("Adding Directory " + Settings.copyLocations[ i ] + " to DropDown at item number: " + Integer.toString( i ) );
		...
		directoryJComboBox.addActionListener( this );
		...
		directoryChooserJButton.addActionListener(this);
	...
	/** 
	 *  method that brings up a JFileChooser and places the path of the directory selected into the 
	...
	/**
	 *   this method responds to clicks on the dropdown icon and the three dot icon.
	 */
	public void actionPerformed( ActionEvent e) {
		if ( e.getSource() == directoryChooserJButton ) 
			dirChooser();
		else if ( e.getSource() == directoryJComboBox ) {
			checkDirectory();
		}
	}	
	 ...
	 *  @return 	true if the direcotry was added, false if not.
	 */
