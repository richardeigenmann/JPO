...
/*
CollectionJTreeController.java:  class that creates a JTree for the collection
...
public class CollectionJTreeController
    ...
    /**
     *  a temporary object that serves as a reference to the node that was selected when the popup menu was activated.
    ...
    /**
     * Returns the JTree as a Component so that things that want to display it can do so.
     */
    ...
    public void dragOver( DropTargetDragEvent event ) {
        ...
        if ( ! event.isDataFlavorSupported( JpoTransferable.dmtnFlavor )) {
            ...
            popupPath = collectionJTree.getPathForLocation( event.getLocation().x, event.getLocation().y );
            if ( popupPath != null ) {
...
//		new JarDistillerJFrame( popupNode );
    ...
    private class CollectionMouseAdapter extends MouseAdapter {
        ...
        private void maybeShowPopup( MouseEvent e ) {
            if ( e.isPopupTrigger() ) {
                popupPath = ((JTree) e.getSource()).getPathForLocation(e.getX(), e.getY());
