...
public class ScreenHelper {
	...
	*   This method returns the bounds of the multihead desktop that a user may have.
	...
	public static Rectangle getTopLeftScreenBounds() {
		if ( isXinerama() ) {
			Rectangle xsb = getXineramaScreenBounds();
			return new Rectangle ( xsb.x, xsb.y, (int) xsb.width / getNumberOfScreenDevices(), (int) xsb.height / 2 );
		} else {
			Rectangle r = getPrimaryScreen().getBounds();
			return new Rectangle( r.x, r.y, (int) r.width/2, (int) r.height/2 );
		}
	}
	...
	public static Rectangle getBottomLeftScreenBounds() {
		if ( isXinerama() ) {
			Rectangle xsb = getXineramaScreenBounds();
			return new Rectangle ( xsb.x, xsb.y + ( (int) xsb.height / 2 ), (int) xsb.width / getNumberOfScreenDevices(), (int) xsb.height / 2 );
		} else {
			Rectangle r = getPrimaryScreen().getBounds();
			return new Rectangle( r.x, r.y + ((int) r.height/2), (int) r.width/2, (int) r.height/2 );
		}
	}
