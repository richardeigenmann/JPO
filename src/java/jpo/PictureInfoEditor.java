...
public class PictureInfoEditor
    ...
    public PictureInfoEditor( SortableDefaultMutableTreeNode editNode ) {
        ...
        creationTimeJTextField.addFocusListener( new FocusListener() {
            public void focusGained( FocusEvent e ) { parseit(); }
            public void focusLost(  FocusEvent e ) { parseit(); }
    ...
    public static String selectedJListCategoriesToString( JList theList ) {
        ...
        if ( ! theList.isSelectionEmpty() ) {
            ...
            for ( int i = 0; i < selectedCategories.length; i++ ) {
                if ( ( selectedCategories[ i ] == setupCategories )
                || ( selectedCategories[ i ] == noCategories ) ) {
                    // skip them
                } else if ( i == 0 ) {
    ...
    private void okButtonEvent() {
        ...
        boolean nothingChanged =
                pi.getDescription().equals( descriptionJTextArea.getText() )
                && pi.getCreationTime().equals( creationTimeJTextField.getText() )
                && pi.getHighresLocation().equals( highresLocationJTextField.getText() )
                && pi.getLowresLocation().equals( lowresLocationJTextField.getText() )
                && pi.getComment().equals( commentJTextField.getText() )
                && pi.getPhotographer().equals( photographerJTextField.getText() )
                && pi.getFilmReference().equals( filmReferenceJTextField.getText() )
                && pi.getCopyrightHolder().equals( copyrightHolderJTextField.getText() );
