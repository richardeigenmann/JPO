...
public class BrokenThumbnailImage {
    ...
    public static void getImage(final ScalablePicture scalablePicture) throws IOException {
        try {
            final var resource = BrokenThumbnailImage.class.getClassLoader().getResource(BROKEN_THUMBNAIL_FILENAME);
            assert resource != null;
            final var resourceAsURI = resource.toURI();
            final var brokenThumbnailImageFile = new File(resourceAsURI);
