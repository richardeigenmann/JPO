...
public class ScalablePicture
    ...
    public ScalablePicture() {
        setStatus(SCALABLE_PICTURE_UNINITIALISED, JpoCoreResources.getResource("ScalablePictureUninitialisedStatus"));
        setScaleFactor(1);
    ...
    public void loadAndScalePictureInThread( final String sha256, final File file, final int priority, final double rotation ) {
        ...
        setStatus(SCALABLE_PICTURE_LOADING, JpoCoreResources.getResource("ScalablePictureLoadingStatus"));
        scaleAfterLoad = true;
    ...
    private BufferedImage getScaledImage(final BufferedImage sourcePicture, final double scaleFactor) {
        var resultImage = sourcePicture;
        double factor = getFactor(scaleFactor);
        final var afStep = AffineTransform.getScaleInstance(factor, factor);
        final var opStep = new AffineTransformOp(afStep, getAffineTransformOp());
        for (var i = 0; i < getScaleSteps(); i++) {
            Point2D pStep = new Point2D.Float(resultImage.getWidth(), resultImage.getHeight());
            pStep = afStep.transform(pStep, null);
            final var size = new Dimension((int) Math.rint(pStep.getX()), (int) Math.rint(pStep.getY()));
            var imageType = sourcePicture.getType();
            ...
            final var biStep = new BufferedImage(size.width, size.height, imageType);
