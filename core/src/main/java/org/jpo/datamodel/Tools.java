...
public class Tools {
    ...
    public static String freeMemory() {
        ...
        return (JpoCoreResources.getResource("freeMemory") + freeMemory + "MB/" + totalMemory + "MB/" + maxMemory + "MB");
    }
    ...
    /**
     * Utility to extract a resource to a temporary file which is deleted on JVM exit
     */
    public static File copyResourceToTempFile(String resourcePath) throws IOException {
        try (final var is = Tools.class.getResourceAsStream(resourcePath)) {
            if (is == null) {
                throw new IOException("Resource not found: " + resourcePath);
            }
            Path tempFile = java.nio.file.Files.createTempFile("JpoImageIoTest", '.' + FilenameUtils.getExtension(resourcePath));
